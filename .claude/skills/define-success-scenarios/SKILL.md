...
# SKILL: define-success-scenarios
...
- verify that no materially required downstream design layer still blocks safe validation derivation
...
It does not decompose work into tasks, instantiate design, design implementation, or generate later-stage artifacts.
...
- no materially required design-foundation dependency still blocks validation for the active execution line
...
- the current execution line still depends on `design-foundation` before validation can be derived honestly
...
If design readiness is still materially unresolved for the active execution line, return to:
...
- stop
- do not derive validation
- return control to `define-prd`

A prior PRD completion claim does not override this recheck.

---

## Design-Readiness Gate Before Validation

PRD stability alone is not always enough to justify validation derivation.

Before creating or updating the validation artifact, this skill must explicitly evaluate whether the active execution line still depends on a materially required `design-foundation` layer.

This gate becomes materially relevant when one or more of the following is true:

- shared user-facing surface consistency matters across multiple pages, screens, or states
- reusable component-state logic materially affects downstream correctness
- accessibility should be materially validated against a structured design system rather than improvised later
- validation scenarios would otherwise require implicit assumptions about tokens, theming, component mapping, or surface rules
- premium, trust-sensitive, or differentiated interface quality materially affects what “correct” behavior should mean for the current execution line

A correct pre-validation decision must be able to state one of the following:

- `design-foundation` is materially required and already exists for the current execution line
- a governed equivalent already exists and remains valid for the current execution line
- `design-foundation` is genuinely unnecessary for the current bounded validation step
- design readiness is still materially unresolved, so validation must remain blocked

If validation would still depend on unresolved design-foundation questions, this skill must stop.

It must not derive validation by silently assuming those answers.
...
It does not automatically satisfy the project-level design layer that later validation, tasking, or handoff may materially depend on.
...
as automatic proof that downstream validation is safe to derive.

If validation still depends on a concrete implementation-facing design layer, this skill must return control to `define-design-foundation` rather than continuing by optimistic interpretation.

---

## Validation Derivation Role

Once readiness is established, this skill must transform the stabilized scope into a bounded validation artifact.
...
This skill must prefer product-observable validation over implementation-dependent validation.

If a candidate validation point would only make sense by assuming framework, storage, runtime, architecture choices, or unresolved design-foundation structure that are not yet governed for the active execution line, that point must not be converted into a validation requirement.
...
Validation scenarios must be grounded in the active PRD and, when materially relevant, in the governed design basis of the current execution line.
...
unless those are already materially grounded in the PRD or in an already-governed design layer for the same execution line.
...
## Technical Baseline Non-Propagation Rule

This skill must not use validation derivation as a place to propagate repository-level technical defaults into the validation artifact.

Validation must remain grounded in product behavior and observable outcomes.

This skill must not introduce technical baseline content into validation merely because the repository contains preferred defaults such as:

- frontend framework defaults
- UI library defaults
- styling defaults
- backend/runtime defaults
- persistence/database defaults

Repository-level defaults are not, by themselves, sufficient reason to include technical assumptions or dependencies in the validation artifact.

Unless a technical baseline has already been materially established for the active execution line through a governed technical-baseline decision, this skill must not:

- name framework choices in validation assumptions
- name database choices in validation assumptions
- name runtime or hosting choices in validation assumptions
- describe implementation architecture as validation fact
- convert repository preferences into validation dependencies

If a validation scenario truly depends on a technical baseline that is still not governed, this skill must not silently fill that gap.

It must instead:

- keep the validation artifact conservative
- focus on observable product behavior
- surface the dependency as open only when the template supports that treatment truthfully

Validation is not the place where repository defaults become execution-line facts.
...
- rewrite the active design basis as though downstream tasking had already started
...
1. confirm PRD readiness and design-readiness sufficiency for the active execution line
2. create or update the validation artifact in the canonical active validation location
3. confirm the validation artifact is materially present and coherently reflects the active PRD
...
Immediately after validation closure, `Current Phase` must still describe the live operational moment truthfully.

This skill may identify task decomposition as the next allowed governed step, but it must not advance `Current Phase` to `tasks` unless a canonical active task artifact already exists and the state is being reconstructed from a materially active downstream artifact rather than advanced speculatively.

A narrated future tasking step is not sufficient to promote the live phase.
...
Its role is limited to deriving the validation artifact from a stabilized PRD and an already-sufficient upstream basis.
...
If materially required design readiness is still unresolved for the active execution line:

- stop validation derivation
- identify the missing design dependency clearly
- return control to `define-design-foundation`
- do not derive validation on top of unresolved design assumptions
...
A correct `define-success-scenarios` must answer:

- is the PRD ready enough for validation?
- is any materially required downstream design layer already sufficient for safe validation?
...
Its job is to derive a canonical validation artifact from a stabilized PRD and a sufficiently governed upstream basis, close the validation step coherently when possible, and then stop.
