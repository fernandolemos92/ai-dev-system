...
# SKILL: implement-approved-scope
...
handoff  
↓  
**implement-approved-scope ← this skill**  
↓  
review-delivery  
↓  
closure decision
...
This is a **MUST** verification, not an optional check.

## Required Verification Steps

### 1. Artifact Location Check

- read the actual canonical directory contents (`project/tasks/active/`, `project/handoff/active/`, etc.)
- verify the active artifacts exist at the claimed paths
- if any artifact is missing: **STOP immediately**

### 2. State vs. Filesystem Consistency

- compare `PROJECT_STATE.md` field values against actual filesystem
- verify that `Current Phase`, `Execution Unlocked`, `Active Task`, and `Active Handoff` match what physically exists
- if `Current Phase` describes `implementation` but files are not in expected locations: **STOP and report**
- if `Active Handoff` references a non-existent file: **STOP immediately**

### 3. Boundary Validity Check

- verify the active handoff still matches the current task boundary
- if scope has materially drifted from what the handoff describes: **STOP and return to `prepare-handoff`**
...
| Type | Example | Action |
|------|---------|--------|
| Critical | artifact missing, wrong path, wrong format | STOP immediately |
| Minor | stale timestamps, cosmetic inconsistencies | flag and continue with caution |
| Ambiguous | state describes later phase than files reflect | STOP and escalate |

## Must Not

- attempt self-resolution of divergences
- mutate `PROJECT_STATE.md` to manufacture entry readiness
- treat a missing artifact as "probably okay to continue"
- implement on top of inconsistent state
...
## No Implicit Bootstrap Rule

This skill must not silently bootstrap a new application baseline as part of bounded feature implementation.

If the implementation baseline does not materially exist yet, this skill must stop and return control to:

- `bootstrap-implementation-baseline`

Feature implementation may only proceed once the governed baseline is both:
- authorized
