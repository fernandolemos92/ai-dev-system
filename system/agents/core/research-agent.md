...
It may inform experience direction, but it must not replace the relevant design specialist or workflow.
...
This agent operates as the canonical entry point for a modular internal research package.

Research in this system is structured as a single canonical agent with sub-guidance layers, not as multiple competing agents.

### Research Squad Structure

The research squad is located at:

`system/agents/research/`

It contains:

- `research-agent.md` — canonical entry point (this file)
- `README.md` — module selection guidance
...
### How This Agent Uses the Research Squad

When this agent is invoked for a bounded research pass:

1. **Start from the research question** — use `framing.md` to clarify the decision question before expanding investigation

2. **Load only relevant sub-guidance** — select the research sub-modules that are materially relevant for the current bounded research pass. Not every research pass uses every layer.

3. **Apply bounded extraction** — extract only the guidance relevant to the current bounded need. Do not dump the entire research squad into context blindly.

4. **Produce decision-ready synthesis** — use `decision-synthesis.md` as the output layer to convert findings into a form that supports the next bounded decision.

### Module Selection Examples

A research pass for a backend data pipeline project may only need:
- `framing.md` → `domain-operations.md` → `decision-synthesis.md`

A research pass for a user-facing SaaS product may need:
- `framing.md` → `market-landscape.md` → `benchmark-patterns.md` → `competitor-analysis.md` → `journey-and-ux-signals.md` → `insight-generation.md` → `decision-synthesis.md`
...
### Sub-Guidance File Rules

- Sub-guidance files are not registered as separate canonical agents
- Sub-guidance files are loaded selectively by this agent based on decision relevance
- The research squad maintains a single canonical identity: `research`
- Do not invoke sub-guidance files as if they were independent agents
